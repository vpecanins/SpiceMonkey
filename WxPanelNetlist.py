...
class WxPanelNetlist(wx.Panel):
    def __init__(self, parent, root: WxMainWindow):
        ...
        self.btn_parse_solve = wx.Button(self.panel1, label="Parse && Solve")
        ...
        self.sizer1.Add(self.btn_parse_solve, 0, wx.EXPAND | wx.ALL, 1)
        ...
        self.txt_spice_optimized = wx.TextCtrl(self.panel2, style=wx.TE_MULTILINE | wx.TE_READONLY)
    ...
    def event_handler_btn_parse_solve(self, event):
        assert (not self.solver_lock.locked())  # Buttons are disabled while solving so should never be called
    ...
    def event_handler_btn_optimize(self, event):
        if self.btn_optimize.GetLabelText() == "Stop":
            ...
            self.worker_thread = threading.Thread(target=self.engine.optimizer_thread_func)
            self.worker_thread.start()
        ...
        while True:
            ...
            while True:
                ...
                if len(self.changed_flag) == 0:
                    # Not changed, exit the parser Do-while loop
                    break
                else:
                    self.root.app_state.netlist = self.txt_spice.GetValue()
            ...
            if len(self.changed_flag) == 0:
                # Not changed, exit the parser & solve Do-while loop
                break
            else:
