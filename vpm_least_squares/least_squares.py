...
from vpm_least_squares.trf import trf
from vpm_least_squares.dogbox import dogbox
from scipy.optimize._lsq.common import EPS, in_bounds, make_strictly_feasible


TERMINATION_MESSAGES = {
    -2: "Stopped by user (outfun returned True).",
...
def least_squares(
    ...
    variables) and the loss function rho(s) (a scalar function), `vpm_least_squares`
    ...
    >>> from scipy.optimize import vpm_least_squares
    >>> x0_rosenbrock = np.array([2, 2])
    >>> res_1 = vpm_least_squares(fun_rosenbrock, x0_rosenbrock)
    >>> res_1.x
    array([ 1.,  1.])
    >>> res_1.cost
    ...
    to `vpm_least_squares` in the form ``bounds=([-np.inf, 1.5], np.inf)``.
    ...
    >>> res_2 = vpm_least_squares(fun_rosenbrock, x0_rosenbrock, jac_rosenbrock,
    ...
    ...
    >>> res_3 = vpm_least_squares(fun_broyden, x0_broyden,
    ...
    >>> res_lsq = vpm_least_squares(fun, x0, args=(t_train, y_train))

    Now compute two solutions with two different robust loss functions. The
    parameter `f_scale` is set to 0.1, meaning that inlier residuals should
    not significantly exceed 0.1 (the noise level used).

    >>> res_soft_l1 = vpm_least_squares(fun, x0, loss='soft_l1', f_scale=0.1,
    ...                             args=(t_train, y_train))
    >>> res_log = vpm_least_squares(fun, x0, loss='cauchy', f_scale=0.1,
    ...                         args=(t_train, y_train))
    ...
    complex variables can be optimized with ``vpm_least_squares()``. Consider the
    following function:
    ...
    >>> from scipy.optimize import vpm_least_squares
    >>> res_wrapped = vpm_least_squares(f_wrap, (0.1, 0.1), bounds=([0, 0], [1, 1]))
